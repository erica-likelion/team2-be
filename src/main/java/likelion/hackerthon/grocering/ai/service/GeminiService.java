...
public class GeminiService {
    ...
    private String buildRecipePrompt(String userOnboardingData, String storeProducts) {
        return String.format("""
            ...
            위 정보를 바탕으로 사용자가 가장 좋아할 만한 최상의 레시피 1개만을 추천해주세요:
            1. 해당 식료품점에서 구매 가능한 재료만 사용
            2. 사용자 선호도를 충분히 반영한 최적의 레시피 선택
            3. 조리법을 단계별로 자세하고 상세하게 설명
            4. 예상 조리 시간과 난이도 표시
            5. 필요한 재료와 대략적인 가격 정보 포함
            6. 왜 이 레시피를 추천하는지 명확한 이유 제시

            다음과 같은 형식으로 답변해주세요:

            **추천 레시피: [요리명]**
            - 요리 종류: [한식/중식/일식/양식/할랄/러시아 등]
            - 난이도: [쉬움/보통/어려움]
            - 조리 시간: [00분]
            - 설명: [요리에 대한 간단한 설명]

            **추천 이유:**
            [사용자의 선호도와 이 식료품점의 특성을 고려했을 때, 왜 이 레시피가 가장 적합한지에 대한 2-3문장의 설명]
